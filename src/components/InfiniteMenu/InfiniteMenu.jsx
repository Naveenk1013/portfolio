...
void main() {
    // 0. Use the instance ID to figure out which item this is
    int itemIndex = vInstanceId % uItemCount;

    // 1. Calculate which cell in the texture atlas to use
    int cellsPerRow = uAtlasSize;
    int cellX = itemIndex % cellsPerRow;
    int cellY = itemIndex / cellsPerRow; // Integer division

    // 2. Calculate UV coordinates for that cell
    vec2 cellSize = vec2(1.0) / vec2(float(cellsPerRow));
    vec2 cellOffset = vec2(float(cellX), float(cellY)) * cellSize;

    // 3. Aspect ratio correction for the image within the cell
    //    (Assuming we want 'cover' style fitting)
    ivec2 texSize = textureSize(uTex, 0);
    float imageAspect = float(texSize.x) / float(texSize.y);
    float containerAspect = 1.0; // Square cell
    
    float scale = max(imageAspect / containerAspect, 
                     containerAspect / imageAspect);
    
    // Transform UVs to center/scale the image
    vec2 st = vec2(vUvs.x, 1.0 - vUvs.y); // Flip Y if needed
    st = (st - 0.5) * scale + 0.5;
    
    // Clamp to avoid bleeding (optional, depending on visual needs)
    st = clamp(st, 0.0, 1.0);
    
    // Map to the specific cell in the atlas
